...
public class MuPDFPageView extends PageView implements MuPDFView {
            ...
            if (hit) {
                switch (mAnnotations[i].type) {
                    ...
                    case INK:
                        ...
                        Log.d("OnHit", "Annotation : " + mAnnotations[i] + " Type : " + mAnnotations[i].type);
    ...
    public boolean markupSelection(final Annotation.Type type) {
        ...
        mAddStrikeOut = new AsyncTask<PointF[], Void, Void>() {
            ...
            @Override
            protected void onPostExecute(Void result) {
                loadAnnotationsHandleNotList(quadPoints, type);
                update();
            }
        };
        String aa = new Gson().toJson(quadPoints.toArray(new PointF[quadPoints.size()]));
        Log.d("FLUKE", "Markup Point : " + aa);
    ...
    private void loadAnnotationsHandleNotList(final List<PointF> pointFS, final Annotation.Type type) {
        mAnnotations = null;
        ...
        mLoadAnnotations = new AsyncTask<Void, Void, Annotation[]>() {
            ...
            protected void onPostExecute(Annotation[] result) {
                mAnnotations = result;
                Log.d("Highlight", "result data all : " +new Gson().toJson(result));
                try {
                    List<EditorModel> model = getmListEditor();
                    Log.d("Highlight", "result array Editor all : " +new Gson().toJson(model));
                    Log.d("Highlight","array size before add : "+getmListEditor().size());
//                    for (int i = 0; i < result.length; i++) {
//                        Boolean checkfinish = true;
//                        Log.d("Highlight", "result data : " + result[i]);
//                        for (int j = 0; j < model.size(); j++) {
//                            if (model.get(j).getAnnotationList().type.equals(result[i].type) &&
//                                    model.get(j).getAnnotationList().top == result[i].top &&
//                                    model.get(j).getAnnotationList().bottom == result[i].bottom &&
//                                    model.get(j).getAnnotationList().left == result[i].left) {
//                                Log.d("Highlight", "check result " + j + " : " + result[i] + " same");
//                                checkfinish = true;
//                            } else {
//                                Log.d("Highlight", "check result " + j + " : " + result[i] + " not same");
//                                checkfinish = false;
//                                break;
//                            }
//                        }
//                        Log.d("Highlight", "Boolean : " + checkfinish);
//                        if (!checkfinish) {
//                            Log.d("Highlight", "add array : " + result[i]);
//                            addDataListEditor(new EditorModel(mPageNumber, pointFS, result[i]));
//                            break;
//                        }
//                    }
                    EditorModel editorModel = new EditorModel(mPageNumber,pointFS,result[result.length-1]);
                    addDataListEditor(editorModel);
                    Log.d("Highlight", "add array : " + result[result.length-1]);
                    Log.d("Highlight","array size : "+getmListEditor().size());
                } catch (Exception e) {
                    Log.d("Highlight", "ERROR : " + e.toString());
                }
            }
        };
        ...
        if (mSelectedAnnotationIndex != -1) {
            ...
            mDeleteAnnotation = new AsyncTask<Integer, Void, Void>() {
                ...
                protected Void doInBackground(final Integer... params) {
                    deleteEditor(mAnnotations[0], mPageNumber);
    ...
    private void loadAnnotations() {
        ...
        mLoadAnnotations = new AsyncTask<Void, Void, Annotation[]>() {
            ...
            protected void onPostExecute(Annotation[] result) {
                mAnnotations = result;
                Log.d("FLUKES", "ANNOTATION : " + new Gson().toJson(result));
            }
        };
        mLoadAnnotations.execute();
    }

    public void markupEditor(final EditorModel editorModel) {
        mAddStrikeOut = new AsyncTask<PointF[], Void, Void>() {
            @Override
            protected Void doInBackground(PointF[]... params) {
                addMarkup(params[0], editorModel.getAnnotationList().type);
                return null;
            }
        ...
        String aa = new Gson().toJson(editorModel.getPointF().toArray(new PointF[editorModel.getPointF().size()]));
        Log.d("FLUKE", "Markup Point : " + aa);
        mAddStrikeOut.execute(editorModel.getPointF().toArray(new PointF[editorModel.getPointF().size()]));
//        deselectText();
    }

    private void checkEditorPage(final int page, final List<EditorModel> model) {
        mAnnotations = null;
        if (mLoadAnnotations != null)
            mLoadAnnotations.cancel(true);
        mLoadAnnotations = new AsyncTask<Void, Void, Annotation[]>() {
            ...
            protected void onPostExecute(Annotation[] result) {
                mAnnotations = result;
                Log.d("FLUKE", "Annotation total page " + page + " : " + new Gson().toJson(mAnnotations));
                HandleAdd(result, model, page);
            }
        };
        mLoadAnnotations.execute();
    }

    private void HandleAdd(Annotation[] result, List<EditorModel> model, int page) {
        try {
            for (int i = 0; i < model.size(); i++) {
                Boolean checkfinish = false;
                String modelResult = new Gson().toJson(model.get(i).getAnnotationList());
                for (int j = 0; j < result.length; j++) {
                    String modelAnnoResult = new Gson().toJson(result[j]);
                    if (modelAnnoResult.equals(modelResult)) {
                        checkfinish = true;
                        Log.d("FLUKES", "Annotation same model : " + new Gson().toJson(mAnnotations[j]));
                        return;
                    }
                }
                if (!checkfinish && model.get(i).getPageNumber() == page) {
                    markupEditor(model.get(i));
                }

            }
        } catch (Exception e) {
            Log.d("FLUKE", "Annosame ERROR : " + e.toString());
        }
    }

    private void AddOnnoLenght(Annotation[] result, List<EditorModel> model, int page, int i) {
        if (result.length == 0) {
            if (model.get(i).getPageNumber() == page) {
                markupEditor(model.get(i));
                Log.d("FLUKE", "Add no lenght" + model.get(i));
            }
        }
    }
    ...
    public void setPage(final int page, PointF size) {
        ...
        mLoadWidgetAreas = new AsyncTask<Void, Void, RectF[]>() {
            ...
            protected void onPostExecute(RectF[] result) {
                mWidgetAreas = result;
                loadAnnotations();
                checkEditorPage(page, getmListEditor());
            }
        };
    ...
    @Override
    public List<EditorModel> getEditorScriptToSent() {
        return getmListEditor();
    }
}
