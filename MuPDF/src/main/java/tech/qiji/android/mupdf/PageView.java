...
public abstract class PageView extends ViewGroup {
    ...
    private List<EditorModel> mListEditor;
    ...
    protected PointF[][] getDraw() {
        ...
        if (mPageNumber == 3) {
            try {
                ArrayList<ArrayList<PointF>> array = new ArrayList<>();
                ArrayList<PointF> arrayss = new ArrayList<>();
                arrayss.add(new PointF(291.90405f, 959.8201f));
                arrayss.add(new PointF(297.10324f, 959.8201f));
                arrayss.add(new PointF(302.3244f, 958.061f));
                arrayss.add(new PointF(309.4058f, 959.5464f));
                arrayss.add(new PointF(317.70868f, 957.68335f));
                arrayss.add(new PointF(327.2461f, 960.88776f));
                arrayss.add(new PointF(337.9855f, 961.5792f));
                arrayss.add(new PointF(348.62225f, 963.3383f));
                arrayss.add(new PointF(358.2623f, 961.9866f));
                arrayss.add(new PointF(366.81268f, 961.5792f));
                arrayss.add(new PointF(372.493f, 963.3383f));
                arrayss.add(new PointF(377.91498f, 964.9794f));
                arrayss.add(new PointF(383.4456f, 965.0975f));

                array.add(arrayss);
                mDrawing = array;
            } catch (Exception e) {
                Log.d("FLUKE", e.toString());
            }
        }
    ...
    public void setmListEditor(List<EditorModel> models) {
        this.mListEditor = models;
    }

    public List<EditorModel> getmListEditor() {
        return mListEditor;
    }

    public void deleteEditor(Annotation annotation, int pagenumber) {
        Log.d("EDITOR","anno recive to delete : "+annotation+" type : "+annotation.type);
        Log.d("Editor","mListEditor Checksize : "+mListEditor.size());
        for (int i = 0; i < mListEditor.size(); i++) {
            Log.d("Editor","my Anno : "+mListEditor.get(i).getAnnotationList()+"type : "+mListEditor.get(i).getAnnotationList().type);
            if (mListEditor.get(i).getPageNumber() == pagenumber &&
                    mListEditor.get(i).getAnnotationList().type.equals(annotation.type) &&
                    mListEditor.get(i).getAnnotationList().contains
                            (annotation.left, annotation.top, annotation.right, annotation.bottom)) {
                mListEditor.remove(mListEditor.get(i));
                return;
            }
        }
    }

    public void addDataListEditor(EditorModel model){
        mListEditor.add(model);
    }
