...
public class MuPDFCore {
    ...
    private native void drawPage(Bitmap bitmap, int pageW, int pageH, int patchX, int patchY,
                                 int patchW, int patchH, long cookiePtr);

    private native void updatePageInternal(Bitmap bitmap, int page, int pageW, int pageH, int patchX,
                                           int patchY, int patchW, int patchH, long cookiePtr);
    ...
    public MuPDFCore(Context context, String filename) throws Exception {
        ...
        if (globals == 0) {
            throw new Exception(
                    String.format(context.getString(R.string.cannot_open_file_Path), filename));
        }
    ...
    public synchronized void drawPage(Bitmap bm, int page, int pageW, int pageH, int patchX,
                                      int patchY, int patchW, int patchH, Cookie cookie) {

        gotoPage(page);
        drawPage(bm, pageW, pageH, patchX, patchY, patchW, patchH, cookie.cookiePtr);
    }

    public synchronized void updatePage(Bitmap bm, int page, int pageW, int pageH, int patchX,
                                        int patchY, int patchW, int patchH, Cookie cookie) {
    ...
    public synchronized TextWord[][] textLines(int page) {
        ...
        for (TextChar[][][] bl : chars) {
            ...
            for (TextChar[][] ln : bl) {
                ...
                if (wds.size() > 0) lns.add(wds.toArray(new TextWord[wds.size()]));
            }
        }
    ...
    public static boolean gprfSupported() {
        if (gs_so_available == false) return false;
        return gprfSupportedInternal();
    }

    public boolean canProof() {
        String format = fileFormat();
        if (format.contains("PDF")) return true;
        return false;
    }
